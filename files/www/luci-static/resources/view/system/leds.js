...
return view.extend({
        ...
        render: function (data) {
                ...
                o.onchange = function (ev, section, value) {
                        for (var i = 0; i < plugins.length; i++) {
                                ...
                                if (plugin.name === value)
                                        this.map.findElement(
                                                "id",
                                                "cbid.system.%s.trigger".format(
                                                        section
                                                )
                                        ).nextElementSibling.innerHTML =
                                                plugin.form.description || "";
                        }
                };
                ...
                s.addModalOptions = function (s) {
                        ...
                        var removeIfNoneActive = function (
                                original_remove_fn,
                                section_id
                        ) {
                        ...
                        for (var optname in opts) {
                                if (
                                        !opts[optname].ucioption ||
                                        optname == opts[optname].ucioption
                                )
                                        continue;
                                opts[optname].remove = removeIfNoneActive.bind(
                                        opts[optname],
                                        opts[optname].remove
                                );
                        }
