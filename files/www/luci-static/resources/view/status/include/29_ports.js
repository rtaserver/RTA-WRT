...
function resolveVLANChain(ifname, bridges, mapping) {
    while (!mapping[ifname]) {
        ...
        if (bridges[m[1]]) {
            ...
            else mapping[ifname] = bridges[m[1]].ports;
...
function buildVLANMappings(mapping) {
    ...
    for (var i = 0, s; (s = bridge_vlans[i]) != null; i++) {
        ...
        var aliases = L.toArray(s.alias),
            ...
            br = (bridges[s.device] = bridges[s.device] || {
                ports: [],
                vlans: {},
                vlan_filtering: true,
            });
        ...
        for (var j = 0; j < ports.length; j++) {
            ...
            if (br.ports.indexOf(port) === -1) br.ports.push(port);
            br.vlans[s.vlan].push(port);
        }
        for (var j = 0; j < aliases.length; j++)
            if (aliases[j] != s.vlan) br.vlans[aliases[j]] = br.vlans[s.vlan];
    }
    for (var i = 0, s; (s = vlan_devices[i]) != null; i++) {
        if (s.type == "bridge") {
            ...
            if (s.vlan_filtering == "0") br.vlan_filtering = false;
            else if (s.vlan_filtering == "1") br.vlan_filtering = true;
            for (var j = 0; j < ports.length; j++)
                if (br.ports.indexOf(ports[j]) === -1) br.ports.push(ports[j]);
            ...
            if (bridges[s.ifname]) {
                ...
                else mapping[s.name] = bridges[s.ifname].ports;
            } else {
    ...
    for (var brname in bridges) {
        ...
        for (var vid in bridges[brname].vlans)
            for (var i = 0; i < bridges[brname].vlans[vid].length; i++)
                resolveVLANChain(
                    bridges[brname].vlans[vid][i],
                    bridges,
                    mapping
                );
    }
    for (var i = 0, s; (s = interfaces[i]) != null; i++) {
        if (!isString(s.device)) continue;
    ...
    if (mapping[ifname]) {
        for (var i = 0; i < mapping[ifname].length; i++) {
            if (!seen[mapping[ifname][i]]) {
                ...
                ports.push.apply(
                    ports,
                    resolveVLANPorts(mapping[ifname][i], mapping, seen)
                );
            }
        }
    } else {
...
function buildInterfaceMapping(zones, networks) {
    ...
    for (var i = 0; i < networks.length; i++) {
        ...
        for (var j = 0; j < ports.length; j++) {
            portmap[ports[j]] = portmap[ports[j]] || {
                networks: [],
                zones: [],
            };
    ...
    for (var i = 0; i < zones.length; i++) {
        ...
        for (var j = 0; j < networknames.length; j++) {
            ...
            for (var k = 0; k < ports.length; k++) {
                portmap[ports[k]] = portmap[ports[k]] || {
                    networks: [],
                    zones: [],
                };
...
function formatSpeed(carrier, speed, duplex) {
    if (speed && duplex) {
        var d = duplex == "half" ? "\u202f(H)" : "",
            e = E("span", {
                title: _("Speed: %d Mibit/s, Duplex: %s").format(speed, duplex),
            });
        switch (speed) {
            case 10:
                e.innerText = "10\u202fGbE" + d;
                break;
            case 100:
                e.innerText = "100\u202fMbE" + d;
                break;
            case 1000:
                e.innerText = "1\u202fGbE" + d;
                break;
            ...
            case 40000:
                ...
                break;
            default:
                e.innerText = "450\u202fMbE".format(speed, d);
        }
        return e;
    }
    ...
    if (l3dev)
        span.appendChild(
            E("img", {
                title: l3dev.getI18n(),
                src: L.resource(
                    "icons/%s%s.png".format(
                        l3dev.getType(),
                        l3dev.isUp() ? "" : "_disabled"
                    )
                ),
            })
        );
    else span.appendChild(E("em", _("(no interfaces attached)")));
    return span;
}
function renderNetworksTooltip(pmap) {
    ...
    else res[0] = _("Port is not part of any network");
    return E([], res);
}
return baseclass.extend({
    ...
    render: function (data) {
        if (L.hasSystemFeature("swconfig")) return null;
        ...
        return E(
            ...
            {
                style:
                    "display:grid;grid-template-columns:repeat(auto-fit, minmax(70px, 1fr));margin-bottom:1em",
            },
            known_ports.map(function (port) {
                var speed = port.netdev.getSpeed(),
                    ...
                    pzones =
                        pmap && pmap.zones.length
                            ? pmap.zones.sort(function (a, b) {
                                  return L.naturalCompare(
                                      a.getName(),
                                      b.getName()
                                  );
                              })
                ...
                return E(
                    "div",
                    {
                        class: "ifacebox center",
                        style: "margin:.25em;min-width:70px;max-width:100px",
                    },
                    [
                        E(
                            "div",
                        ...
                        E("div", { class: "ifacebox-body" }, [
                            E("img", {
                                src: L.resource("icons/port_%s.gif").format(
                                    carrier ? "up" : "down"
                                ),
                            }),
                            E("br"),
                        ...
                        E(
                            "div",
                            {
                                class: "ifacebox-head cbi-tooltip-container",
                                style: "display:flex",
                            },
                            [
                                E(
                                    [],
                                    pzones.map(function (zone) {
                                        return E("div", {
                                            class: "zonebadge",
                                            style:
                                                "cursor:help;flex:1;height:3px;opacity:" +
                                                (carrier ? 1 : 0.25) +
                                                ";" +
                                ...
                                E("span", { class: "cbi-tooltip left" }, [
                                    renderNetworksTooltip(pmap),
                                ]),
                            ]
                        ),
                        E("div", { class: "ifacebox-body" }, [
                            E(
                                ...
                                [
                                    "\u25b2\u202f%1024.1mB".format(
                                        port.netdev.getTXBytes()
                                    ),
                                    E("br"),
                                    "\u25bc\u202f%1024.1mB".format(
                                        port.netdev.getRXBytes()
                                    ),
                                    E(
                                        "span",
                                        { class: "cbi-tooltip" },
                                        formatStats(port.netdev)
                                    ),
                                ]
                            ),
                        ]),
