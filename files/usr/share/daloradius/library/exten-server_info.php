...
function get_mac_addr($ifname) {
    ...
    foreach ($command_formats as $format) {
        $command = sprintf($format, escapeshellarg($ifname), escapeshellarg($ifname));
        exec($command, $output, $result_code);
        if ($result_code === 0 && preg_match("/^([0-9A-F]{2}\:){5}[0-9A-F]{2}$/i", $output[0] ?? false)) {
            return $output[0] ?? false;
        }
    }
    return "(n/d)";
...
?>
<h3><center>𝗚𝗲𝗻𝗲𝗿𝗮𝗹 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻</h3>
...
<h3><center>𝗠𝗲𝗺𝗼𝗿𝘆 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻</h3>
<table class="summarySection">
  <tr>
...
<h3><center>𝗛𝗮𝗿𝗱𝗱𝗿𝗶𝘃𝗲 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻</h3>
<table class="summarySection">
  <tr>
...
<h3><center>𝗡𝗲𝘁𝘄𝗼𝗿𝗸 𝗜𝗻𝘁𝗲𝗿𝗳𝗮𝗰𝗲𝘀</h3>
<?php
